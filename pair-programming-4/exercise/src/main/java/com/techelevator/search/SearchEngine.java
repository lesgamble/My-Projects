...
public class SearchEngine {
	...
	public void indexFiles() throws SearchEngineException, Exception {
		...
		List<String> fileList = sd.getFiles();
		for (int i = 0; i < fileList.size(); i++) {
			File tempFile = new File(sd.getFolder() + "/" + fileList.get(i));
			try (Scanner sc = new Scanner(tempFile)) {
				while (sc.hasNextLine()) {
					indexWords(i, sc.nextLine());
				}
			} catch(Exception e) {
				throw new SearchEngineException("Search engine exception has occurred!");
			}
		}
